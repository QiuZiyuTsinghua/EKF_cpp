...
int main() {
    std::cout << "=== Extended Vehicle Dynamics EKF Test (7-State Model) ===" << std::endl;
    ...
    ekf.setStateJacobianFunction([m, Iz, Cf, Cr, lf, lr, track_f, track_r, tau_fy]
        ...
        // 车辆动力学雅可比项
        J(0, 1) += dt * gamma;          // ∂v̇_x/∂v_y
        J(0, 2) += dt * vy;             // ∂v̇_x/∂γ
        J(1, 0) += -dt * gamma;         // ∂v̇_y/∂v_x
        J(1, 2) += -dt * vx;            // ∂v̇_y/∂γ
        
        // 横向力对车辆动力学的贡献
        J(0, 3) += dt * (-sin_delta) / m;   // 前轮横向力对纵向动力学
        J(0, 4) += dt * (-sin_delta) / m;
        J(1, 3) += dt * cos_delta / m;      // 横向力对横向动力学
        J(1, 4) += dt * cos_delta / m;
        J(1, 5) += dt * 1.0 / m;            // 后轮横向力
        J(1, 6) += dt * 1.0 / m;
    ...
    ekf.setMeasurementFunction([m, Iz, Cf, Cr, lf, lr, track_f, track_r]
        ...
        // 假设当前时刻的控制输入 (简化)
        double delta = 0.05 * sin(0.5);  // 简化：固定转向
        double Fx_fl = 100.0;
        double Fx_fr = 100.0;
        double Fx_rl = 200.0;
        double Fx_rr = 200.0;
    ...
    ekf.setMeasurementJacobianFunction([m, Iz, Cf, Cr, lf, lr, track_f, track_r]
        ...
        // 加速度测量雅可比
        H(0, 3) = -sin_delta / m;      // ax对前轮横向力
        H(0, 4) = -sin_delta / m;
        H(1, 3) = cos_delta / m;       // ay对横向力
        H(1, 4) = cos_delta / m;
        H(1, 5) = 1.0 / m;
        H(1, 6) = 1.0 / m;
        H(2, 2) = 1.0;                 // 横摆角速度直接测量
        ...
        // 轮速测量雅可比 (简化版本)
        H(3, 0) = 1.0;  H(3, 2) = -track_f/2;  // 前左轮
        H(4, 0) = 1.0;  H(4, 2) = track_f/2;   // 前右轮
        H(5, 0) = 1.0;  H(5, 2) = -track_r/2;  // 后左轮
        H(6, 0) = 1.0;  H(6, 2) = track_r/2;   // 后右轮
        
        return H;
    });
    
    // 仿真参数
    std::mt19937 gen(123);  // 随机数生成器
    ...
    std::cout << "\nStarting simulation with:" << std::endl;
    std::cout << "- Time step: " << dt << "s" << std::endl;
    std::cout << "- Total steps: " << steps << std::endl;
    std::cout << "- Vehicle mass: " << m << "kg" << std::endl;
    std::cout << "- Front/Rear cornering stiffness: " << Cf << "/" << Cr << "N/rad" << std::endl;
    ...
    for (int i = 0; i < steps; i++) {
        ...
        // 生成控制输入 - 模拟转向和制动场景
        Eigen::VectorXd u(controlSize);
        double steering_input = 0.1 * sin(0.5 * t);  // 正弦转向输入
        double brake_force = (t > 2.0 && t < 3.0) ? -500.0 : 1000.0;  // 2-3秒制动
        
        u << steering_input,    // 转向角
             brake_force,       // 左前轮纵向力
             brake_force,       // 右前轮纵向力
             brake_force,       // 左后轮纵向力
             brake_force;       // 右后轮纵向力
        ...
        try {
            ...
            // 模拟真实测量值 (简化)
            double true_ax = brake_force / m * 4;  // 近似纵向加速度
            double true_ay = true_vy * true_gamma; // 近似横向加速度
            ...
            // 每50步打印一次结果
            if (i % 50 == 0) {
                std::cout << "\n--- Step " << i << " (t=" << t << "s) ---" << std::endl;
                std::cout << "Control: δ=" << u(0)*180/M_PI << "°, Fx=" << u(1) << "N" << std::endl;
                std::cout << "Velocity: vx=" << estimated_state(0) << ", vy=" << estimated_state(1) << " m/s" << std::endl;
                std::cout << "Yaw rate: γ=" << estimated_state(2)*180/M_PI << "°/s" << std::endl;
                std::cout << "Tire forces: Fy_fl=" << estimated_state(3) << ", Fy_fr=" << estimated_state(4) << " N" << std::endl;
                std::cout << "            Fy_rl=" << estimated_state(5) << ", Fy_rr=" << estimated_state(6) << " N" << std::endl;
                std::cout << "Uncertainty (σ): vx=" << sqrt(cov(0,0)) << ", vy=" << sqrt(cov(1,1)) << " m/s" << std::endl;
                std::cout << "                Fy=" << sqrt(cov(3,3)) << " N" << std::endl;
            }
    ...
    Eigen::MatrixXd final_cov = ekf.getCovariance();
    
    std::cout << "\n=== Final Results ===" << std::endl;
    std::cout << "Final state estimate:" << std::endl;
    std::cout << "  Longitudinal velocity: " << final_state(0) << " ± " << sqrt(final_cov(0,0)) << " m/s" << std::endl;
    std::cout << "  Lateral velocity: " << final_state(1) << " ± " << sqrt(final_cov(1,1)) << " m/s" << std::endl;
    std::cout << "  Yaw rate: " << final_state(2)*180/M_PI << " ± " << sqrt(final_cov(2,2))*180/M_PI << " °/s" << std::endl;
    std::cout << "  Front tire forces: " << final_state(3) << ", " << final_state(4) << " N" << std::endl;
    std::cout << "  Rear tire forces: " << final_state(5) << ", " << final_state(6) << " N" << std::endl;
    
    // 检查结果合理性
    bool test_passed = true;
    if (fabs(final_state(0)) < 1.0 || fabs(final_state(0)) > 30.0) {
        std::cout << "WARNING: Longitudinal velocity out of reasonable range!" << std::endl;
        test_passed = false;
    }
    if (fabs(final_state(1)) > 5.0) {
        std::cout << "WARNING: Lateral velocity seems too high!" << std::endl;
        test_passed = false;
    }
    if (fabs(final_state(2)) > 1.0) {
        std::cout << "WARNING: Yaw rate seems too high!" << std::endl;
        test_passed = false;
    }
    
    if (test_passed) {
        std::cout << "\n✓ Extended EKF test PASSED! All estimates are within reasonable ranges." << std::endl;
    } else {
        std::cout << "\n✗ Extended EKF test shows concerning results. Check model parameters." << std::endl;
    }
    
    std::cout << "\n=== Extended Vehicle Dynamics EKF Test Complete ===" << std::endl;
